...
void sha256_final(SHA256_CTX *ctx, uint8_t hash[32]) {
    ...
    // Append the padding bit '1' (0x80)
    size_t idx = ctx->count % 64;
    size_t padlen = (idx < 56) ? (56 - idx) : (120 - idx);

    uint8_t padding[padlen + 8];  // +8 for length
    memset(padding, 0, sizeof(padding));
    padding[0] = 0x80;

    // Append length as big-endian 64-bit
    for (int i = 0; i < 8; ++i) {
        padding[padlen + i] = (bits >> ((7 - i) * 8)) & 0xff;
    }

    sha256_update(ctx, padding, padlen + 8);
